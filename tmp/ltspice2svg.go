...
func ltSymbol2svg(symbolInput, symbolName string, outFile fileInfo, fileWrite bool) string {
	...
	drawSlc = addDrawElements(inLines, drawSlc)
	drawSlc = addWinSym(inLines, drawSlc, symbolName)
...
func ltSpice2svg(LTSpiceInput, symPath, txtMode, dotsMode, fontType string, outFile fileInfo) {
	...
	drawSlc, WiresInt = addWires(inLines, drawSlc, dotsMode)
	drawSlc, logOut = addSymbols(inLines, drawSlc, symbolDefn, txtMode)
	if logOut != "" {
		logOut = logOutWrite(logOut, -1, outFile)
	}
	drawSlc = append(headerSlc, drawSlc...) // after adding symbols, put header at the front of drawSlc
	drawSlc = addLabels(inLines, WiresInt, drawSlc, txtMode)
	drawSlc = addTextLines(inLines, drawSlc, txtMode)
	drawSlc = addDrawElements(inLines, drawSlc)
	drawSlc = append(drawSlc, tail)
	outString = strings.Join(drawSlc, "\n")
...
func getInputFiles(LTSpiceFile string) (string, string) {
	...
	if codeUtf16 {
		fmt.Println("WARNING** LTSpice file is UTF16 encoded")
		logOut = logOut + ".asc file is UTF16 encoded which may cause problems"
	}
...
func addHeader(inLines, drawSlc []string) []string {
	...
	drawSlc = append(drawSlc, header1)
	...
	drawSlc = append(drawSlc, svgLine)
	drawSlc = append(drawSlc, header2)

	return drawSlc
}

func addWires(inLines, drawSlc []string, dotsMode string) ([]string, [][4]int) {
	var WiresInt [][4]int
	...
	for i := 0; i < len(inLines); i++ { // find all wires and add to drawSlc
		if wireMatch.MatchString(inLines[i]) {
			...
			drawSlc = append(drawSlc, svgLine)
			WiresInt = addWiresInt(x1, y1, x2, y2, WiresInt)
	...
	drawSlc = addDots(dotsMode, wiresString, uniqueEndpoints, drawSlc)
	drawSlc = addJumps(WiresInt, drawSlc)
	return drawSlc, WiresInt
}

func addJumps(WiresInt [][4]int, drawSlc []string) []string {
	...
	drawSlc = addJumpsHoriz(WiresHoriz, WiresVert, drawSlc)
	drawSlc = addJumpsDiag(WiresHoriz, WiresVert, WiresDiag, drawSlc)
	return drawSlc
}

func addJumpsHoriz(WiresHoriz, WiresVert [][4]int, drawSlc []string) []string {
...
func addJumpsDiag(WiresHoriz, WiresVert, WiresDiag [][4]int, drawSlc []string) []string {
	...
	for i := 0; i < len(WiresDiag); i++ {
		...
		for j := i + 1; j < len(WiresDiag); j++ {
			...
			if minRotate != "none" {
				drawSlc = jumperDiag(intersect, rotateA, rotateB, minRotate, drawSlc)
			}
		}
...
func jumperDiag(intersect [2]float64, rotateA, rotateB float64, minRotate string, drawSlc []string) []string {
...
func addDots(dotFlag, wiresString string, uniqueEndpoints [][2]string, drawSlc []string) []string {
...
func addSymbols(inLines, drawSlc []string, symbolDefn, txtMode string) ([]string, string) {
	...
	for i := 0; i < len(inLines); i++ { // find all symbols and add to drawSlc
...
func addLabels(inLines []string, WiresInt [][4]int, drawSlc []string, txtMode string) []string {
	...
	for i := 0; i < len(inLines); i++ { // find all labels and add to drawSlc
		if labelMatch.MatchString(inLines[i]) {
			...
			case iopinMatch.MatchString(inLines[i+1]):
				...
				drawSlc = addIoPin(x1, y1, label, ioType, WiresInt, drawSlc, txtMode)
...
func addTextLines(inLines, drawSlc []string, txtMode string) []string {
	var svgLine string
	textMatch := regexp.MustCompile(`(?m)TEXT`)

	for i := 0; i < len(inLines); i++ { // find all text and add to drawSlc
...
func addIoPin(x1, y1, label, ioType string, WiresInt [][4]int, drawSlc []string, txtMode string) []string {
...
func addWiresInt(x1, y1, x2, y2 string, WiresInt [][4]int) [][4]int {
	var tx, ty int
	...
	WiresInt = append(WiresInt, [4]int{Lx1, Ly1, Lx2, Ly2})
	return WiresInt
}
...
func addDrawElements(inLines, drawSlc []string) []string {
	...
	for i := 0; i < len(inLines); i++ { // find all draw elements and add to drawSlc
...
func addWinSym(inLines, drawSlc []string, symName string) []string { // add win/sym lines to symbol output
	var svgLine string
	winSymattrMatch := regexp.MustCompile(`(?m)(WINDOW|SYMATTR)`)
...
func getSymbolDefn(symPath string, inLines []string) (string, string) {
	...
	var dummyFile fileInfo // used to put blank fileInfo variable into function ltSymbol2svg
				...
				if symbolSVG == "" {
					for _, oneDir := range ltSpiceDir {
						...
						if newLogOut == "" { // found the symbol .asy file
							...
							symbolSVG = ltSymbol2svg(symbolFile, symbol, dummyFile, false)
							break
						}
					}
				} else {
